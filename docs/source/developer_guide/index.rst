...
``aiida-jdftx`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-jdftx`` repository (preferably using ``aiida-jdftx`` as the project name)

The documentation is now available at `aiida-jdftx.readthedocs.io <http://aiida-jdftx.readthedocs.io/>`_.
...
.. note::
   ...
   When updating the plugin package to a new version, remember to update the version number both in ``setup.json`` and ``aiida_jdftx/__init__.py``.
